...
class LinkedList:
    ...
    def healthy(self):
        """Testmethod that checks if the list is in a correct state"""
        # O(n) goes through the whole list
        i = 0
        element = self._first
        while element != None:
            element = element.next
        ...
        if self._size == i:
            if i == 0 and self._first == None and self._last == None:
                return True
            if i != 0 and self._last.next == None:
                return True
